...
let bottomRunnerName = document.querySelector(`.bottom-runner-name`);
...
let bottomRunnerTwitch = document.querySelector(`.bottom-runner-twitch`);
...
NodeCG.waitForReplicants(generalRunInfo, nextRunsListSchedule, timer, racePlayers).then(() => {
	...
	racePlayers.on('change', (newValue, oldValue) => {
		...
		// bottomRunnerName.textContent = newValue.players[3];
		bottomRunnerTwitch.textContent = newValue.twitch[3];
	})
